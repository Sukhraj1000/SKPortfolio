...
import { contactDetails, portfolioProfile, socialLinks, storyChapters } from "@/data/portfolio";

const contactSubject = encodeURIComponent("Portfolio conversation from sukhrajkalon.info");
const contactBody = encodeURIComponent(
  "Hi Sukhraj,\n\nI found your portfolio and would like to discuss a relevant opportunity or collaboration.\n\nThanks,",
);
const contactMailto = `mailto:${contactDetails.email}?subject=${contactSubject}&body=${contactBody}`;

export function ContactSection() {
  return (
    <section id="contact" data-chapter="05" aria-labelledby="contact-title" className="pq-ending">
      <div className="pq-ending-scene" data-motion="ending-scene" aria-hidden="true">
        <span className="pq-arrival-grid" />
        <span className="pq-bay-frame is-a">
          <i />
        </span>
        <span className="pq-bay-frame is-b">
          <i />
        </span>
        <span className="pq-overhead-signal">
          <i />
          <i />
          <i />
          <b>Channel open</b>
        </span>
        <span className="pq-comms-panel">
          <i />
          <b>Contact link</b>
          <small>Ready / 05</small>
        </span>
      ...
      <div className="pq-ending-copy" data-motion="record">
        ...
        <p>
          Relevant engineering roles, product work, and collaborations are welcome. The complete
          professional evidence remains easy to scan; the spatial layer gives the journey a
          memorable frame.
        </p>
        ...
        <div className="pq-ending-actions">
          ...
          <a className="pq-button" data-variant="secondary" href={contactMailto}>
